...
add_table(
    ...
    [
        ...
        ["7", "兵庫おでかけ特集（記事メディア）", "お客様", "観光スポット・話題のお店・人気のパン屋さんなど兵庫の情報を記事で発信"],
        ["7b", "注目スポット自動カルーセル", "お客様", "トップで「空き枠が多いエリア」のスポットを自動表示し、地域メディアの印象に"],
        ["7c", "ジャンル検索", "お客様", "カフェ・ランチ・レジャー等のジャンルからスポットを検索"],
        ["8", "レンタカープラン（日帰り/宿泊/訪日）", "お客様", "観光とセットのプラン。神戸空港国際化を見据えたインバウンド対応も"],
        ["9", "入庫スケジュール可視化", "管理者", "全店舗×日付の稼働をAM/PMヒートマップで把握"],
        ["10", "スタッフ体制の把握", "管理者", "各店舗の整備士・フロントの人数と顔ぶれを一覧で把握"],
        ["11", "AI最適化アシスタント", "管理者", "午後誘導・近隣店舗への振り分けを重要度つきで自動提案"],
    ],
    widths=[1, 5, 2.5, 8],
)
...
add_table(
    ["URL", "画面", "内容"],
    [
        ["/", "ホーム（メディア型）", "空きが多いエリアの注目スポットを自動カルーセル表示。ジャンル・記事・エリア・レンタカー・特典を回遊"],
        ["/spots", "ジャンルで探す", "カフェ・ランチ・レジャー等のジャンル＋エリアでスポット検索"],
        ...
        ["/media", "兵庫おでかけ特集", "観光・グルメ・パン屋などの記事一覧＋記事詳細（関連店舗で点検への導線つき）"],
        ["/rentacar", "レンタカー トップ", "日帰り・宿泊・訪日向けプランの入口"],
        ["/rentacar/day・stay", "日帰り／宿泊プラン", "プラン別の一覧ページ"],
...
add_table(
    ...
    [
        ...
        ["/staff/members", "スタッフ体制", "各店舗の人数・顔ぶれ、スタッフ名簿"],
...
add_para("将来的には、予約実績・天候・キャンペーン情報を学習し、LLM（Claude等）による"
         "自然言語提案・自動配信へ拡張する。"
         "（※ スタッフの店舗間移動は運用上現実的でないため機能から除外）",
         italic=True, color=STEEL, size=10)
...
add_bullet("Staff：スタッフ（所属店舗・資格・スキル）")
add_bullet("Article：兵庫おでかけ記事（カテゴリ・エリア・本文・関連店舗）")
add_bullet("RentalPlan：レンタカープラン（日帰り/宿泊・料金・含まれるもの・スポット）")
